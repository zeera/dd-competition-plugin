...
class CompetitionProcess extends AdminHelper
{
    ...
    public static function setTopData()
    {
        ...
        $productData = get_queried_object();
        $productData = wc_get_product( $productData->ID );
        if( wp_style_is('competition-bootstrap-styles') && wp_style_is('competition-single-product-styles') ) {
            if (is_singular('product') ) {
                if ($productData->get_type() == 'competition') {
                    ?>
                            ...
                            <div class="container">
                                <div class="row align-items-center justify-content-center">
                                    <div class="col text-center">
                                        <div class="heading">
                                            ...
                                            <h1 class="text-uppercase fw-bold lh-1" style="color:<?= get_option('top_info_heding_color') ?>"><?= $productData->name; ?></h1>
                                ...
                                <div class="row align-items-center justify-content-center">
                                    <div class="col-12 col-lg-4">
                                        <div
                                            class="top-data fw-medium rounded d-flex flex-row align-items-center justify-content-center text-uppercase"
                                    ...
                                    <div class="col-12 col-lg-4">
                                        <div
                                            ...
                                            style="color:<?= get_option('top_info_text_color'); ?>; background: linear-gradient(180deg, <?= get_option('top_info_background_color_one'); ?> 0%, <?= get_option('top_info_background_color_two'); ?> 100%);">
                                            <?php
                                                $ticketsPerUser = $productData->get_meta('_maximum_ticket_per_user', true) ?? get_option('maximum_ticket_default_per_user'); ?>
        ...
        if( $productData && $productData->get_type() == 'competition' ) {
            if ($totalBought < $maxQtyUser) {
                if( $remainingCredits == $maxQtyUser ) {
                    ...
                    $message = "You only have {$remainingCredits} remaining tickets for this product: <strong>{$productData->name}</strong>. {$cartHtml}";
                    wc_add_notice(__($message, 'woocommerce'), 'error');
                    return false;
                } else {
                    return true;
